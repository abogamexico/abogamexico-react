...
function matchRoutes2(routes, locationArg, basename) {
  ...
  return matchRoutesImpl2(routes, locationArg, basename, false);
}
function matchRoutesImpl2(routes, locationArg, basename, allowPartial) {
  ...
  for (let i = 0; matches == null && i < branches.length; ++i) {
    matches = matchRouteBranch2(branches[i], decoded, allowPartial);
  }
  return matches;
}
...
function matchRouteBranch2(branch, pathname, allowPartial) {
  if (allowPartial === void 0) {
    allowPartial = false;
  }
  let {
...
function resolvePath2(to, fromPathname) {
  ...
  let pathname;
  if (toPathname) {
    toPathname = removeDoubleSlashes2(toPathname);
    if (toPathname.startsWith("/")) {
      pathname = resolvePathname2(toPathname.substring(1), "/");
    } else {
      pathname = resolvePathname2(toPathname, fromPathname);
    }
  } else {
    pathname = fromPathname;
  }
...
var removeDoubleSlashes2 = (path) => path.replace(/\/\/+/g, "/");
var joinPaths2 = (paths) => removeDoubleSlashes2(paths.join("/"));
...
function createRouter2(init) {
  ...
  let inFlightDataRoutes;
  let basename = init.basename || "/";
  let dataStrategyImpl = init.dataStrategy || defaultDataStrategy2;
  let patchRoutesOnNavigationImpl = init.patchRoutesOnNavigation;
  ...
  if (!initialMatches) {
    ...
    if (errors) {
      let idx = initialMatches.findIndex((m) => errors[m.route.id] !== void 0);
      initialized = initialMatches.slice(0, idx + 1).every((m) => !shouldLoadRouteOnHydration2(m.route, loaderData, errors));
    } else {
      initialized = initialMatches.every((m) => !shouldLoadRouteOnHydration2(m.route, loaderData, errors));
    }
  } else {
    initialized = init.hydrationData != null;
  }
  ...
  async function startNavigation(historyAction, location, opts) {
    ...
    let matches = opts != null && opts.initialHydration && state.matches && state.matches.length > 0 && !initialMatchesIsFOW ? (
      // `matchRoutes()` has already been called if we're in here via `router.initialize()`
      state.matches
    ) : matchRoutes2(routesToUse, location, basename);
    let flushSync = (opts && opts.flushSync) === true;
    if (matches && state.initialized && !isRevalidationRequired && isHashChangeOnly2(state.location, location) && !(opts && opts.submission && isMutationMethod2(opts.submission.formMethod))) {
      completeNavigation(location, {
        matches
      }, {
        flushSync
    ...
    let pendingActionResult;
    if (opts && opts.pendingError) {
      pendingActionResult = [findNearestBoundary2(matches).route.id, {
      ...
      if (actionResult.pendingActionResult) {
        let [routeId, result] = actionResult.pendingActionResult;
        if (isErrorResult2(result) && isRouteErrorResponse2(result.error) && result.error.status === 404) {
          pendingNavigationController = null;
          completeNavigation(location, {
            matches: actionResult.matches,
            loaderData: {},
      ...
      flushSync = false;
      fogOfWar.active = false;
      request = createClientSideRequest2(init.history, request.url, request.signal);
    }
    let {
      shortCircuited,
      matches: updatedMatches,
    ...
    completeNavigation(location, _extends3({
      matches: updatedMatches || matches
    }, getActionDataForCommit2(pendingActionResult), {
      loaderData,
      errors
    }));
  }
  async function handleAction(request, location, submission, matches, isFogOfWar, opts) {
    ...
    if (isFogOfWar) {
      ...
      if (discoverResult.type === "aborted") {
        ...
        let boundaryId = findNearestBoundary2(discoverResult.partialMatches).route.id;
        return {
          matches: discoverResult.partialMatches,
          pendingActionResult: [boundaryId, {
            type: ResultType2.error,
    ...
    if (isRedirectResult2(result)) {
      ...
      if (opts && opts.replace != null) {
        ...
        let location2 = normalizeRedirectLocation2(result.response.headers.get("Location"), new URL(request.url), basename, init.history);
        replace2 = location2 === state.location.pathname + state.location.search;
      }
      await startRedirectNavigation(request, result, true, {
        submission,
  ...
  async function handleLoaders(request, location, matches, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace2, initialHydration, flushSync, pendingActionResult) {
    ...
    let routesToUse = inFlightDataRoutes || dataRoutes;
    let [matchesToLoad, revalidatingFetchers] = getMatchesToLoad2(init.history, state, matches, activeSubmission, location, future.v7_partialHydration && initialHydration === true, future.v7_skipActionErrorRevalidation, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionResult);
    cancelActiveDeferreds((routeId) => !(matches && matches.some((m) => m.route.id === routeId)) || matchesToLoad && matchesToLoad.some((m) => m.route.id === routeId));
    pendingNavigationLoadId = ++incrementingLoadId;
    if (matchesToLoad.length === 0 && revalidatingFetchers.length === 0) {
      let updatedFetchers2 = markFetchRedirectsDone();
      completeNavigation(location, _extends3({
        ...
        loaderData: {},
        // Commit pending error if we're short circuiting
        errors: pendingActionResult && isErrorResult2(pendingActionResult[1]) ? {
          [pendingActionResult[0]]: pendingActionResult[1].error
        } : null
      }, getActionDataForCommit2(pendingActionResult), updatedFetchers2 ? {
        fetchers: new Map(state.fetchers)
      } : {}), {
        flushSync
      });
    ...
    revalidatingFetchers.forEach((rf) => fetchControllers.delete(rf.key));
    let redirect3 = findRedirect2(loaderResults);
    if (redirect3) {
      await startRedirectNavigation(request, redirect3.result, true, {
        replace: replace2
      });
    ...
    redirect3 = findRedirect2(fetcherResults);
    if (redirect3) {
      fetchRedirectIds.add(redirect3.key);
      await startRedirectNavigation(request, redirect3.result, true, {
        replace: replace2
    ...
    let {
      loaderData,
      errors
    } = processLoaderData2(state, matches, loaderResults, pendingActionResult, revalidatingFetchers, fetcherResults, activeDeferreds);
    activeDeferreds.forEach((deferredData, routeId) => {
      deferredData.subscribe((aborted) => {
        if (aborted || deferredData.done) {
          activeDeferreds.delete(routeId);
  ...
  function getUpdatedActionData(pendingActionResult) {
    if (pendingActionResult && !isErrorResult2(pendingActionResult[1])) {
      return {
        [pendingActionResult[0]]: pendingActionResult[1].data
      };
    } else if (state.actionData) {
  ...
  async function handleFetcherAction(key, routeId, path, match, requestMatches, isFogOfWar, flushSync, preventScrollReset, submission) {
    ...
    if (isFogOfWar) {
      ...
      if (discoverResult.type === "aborted") {
        ...
        requestMatches = discoverResult.matches;
        match = getTargetMatch2(requestMatches, path);
        if (detectAndHandle405Error(match)) {
          return;
        }
      }
    ...
    state.fetchers.set(key, loadFetcher);
    let [matchesToLoad, revalidatingFetchers] = getMatchesToLoad2(init.history, state, matches, submission, nextLocation, false, future.v7_skipActionErrorRevalidation, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, [match.route.id, actionResult]);
    revalidatingFetchers.filter((rf) => rf.key !== key).forEach((rf) => {
      let staleKey = rf.key;
      let existingFetcher2 = state.fetchers.get(staleKey);
    ...
    let {
      loaderData,
      errors
    } = processLoaderData2(state, matches, loaderResults, void 0, revalidatingFetchers, fetcherResults, activeDeferreds);
    if (state.fetchers.has(key)) {
  ...
  async function handleFetcherLoader(key, routeId, path, match, matches, isFogOfWar, flushSync, preventScrollReset, submission) {
    ...
    if (isFogOfWar) {
      ...
      if (discoverResult.type === "aborted") {
        ...
        matches = discoverResult.matches;
        match = getTargetMatch2(matches, path);
      }
    }
    fetchControllers.set(key, abortController);
    let originatingLoadId = incrementingLoadId;
  ...
  async function startRedirectNavigation(request, redirect3, isNavigation, _temp2) {
    ...
    location = normalizeRedirectLocation2(location, new URL(request.url), basename, init.history);
    let redirectLocation = createLocation2(state.location, location, {
      _isRedirect: true
    });
    if (isBrowser2) {
      let isDocumentReload = false;
    ...
    pendingNavigationController = null;
    let redirectHistoryAction = replace2 === true || redirect3.response.headers.has("X-Remix-Replace") ? Action2.Replace : Action2.Push;
    let {
      formMethod,
      formAction,
      formEncType
    ...
    if (!submission && !fetcherSubmission && formMethod && formAction && formEncType) {
      submission = getSubmissionFromNavigation2(state.navigation);
    }
    let activeSubmission = submission || fetcherSubmission;
    if (redirectPreserveMethodStatusCodes2.has(redirect3.response.status) && activeSubmission && isMutationMethod2(activeSubmission.formMethod)) {
      await startNavigation(redirectHistoryAction, redirectLocation, {
  ...
  async function callDataStrategy(type, state2, request, matchesToLoad, matches, fetcherKey) {
    ...
    let dataResults = {};
    try {
      results = await callDataStrategyImpl2(dataStrategyImpl, type, state2, request, matchesToLoad, matches, fetcherKey, manifest, mapRouteProperties2);
    } catch (e) {
      matchesToLoad.forEach((m) => {
        dataResults[m.route.id] = {
          type: ResultType2.error,
    ...
    for (let [routeId, result] of Object.entries(results)) {
      if (isRedirectDataStrategyResultResult2(result)) {
        let response = result.result;
        dataResults[routeId] = {
          type: ResultType2.redirect,
          response: normalizeRelativeRoutingRedirectResponse2(response, request, routeId, matches, basename, future.v7_relativeSplatPath)
        };
      } else {
        dataResults[routeId] = await convertDataStrategyResultToDataResult2(result);
      }
    }
    return dataResults;
  }
  async function callLoadersAndMaybeResolveData(state2, matches, matchesToLoad, fetchersToLoad, request) {
    ...
    let fetcherResultsPromise = Promise.all(fetchersToLoad.map(async (f) => {
      if (f.matches && f.match && f.controller) {
        let results = await callDataStrategy("loader", state2, createClientSideRequest2(init.history, f.path, f.controller.signal), [f.match], f.matches, f.key);
        let result = results[f.match.route.id];
        return {
          [f.key]: result
        };
    ...
    let loaderResults = await loaderResultsPromise;
    let fetcherResults = (await fetcherResultsPromise).reduce((acc, r) => Object.assign(acc, r), {});
    await Promise.all([resolveNavigationDeferredResults2(matches, loaderResults, request.signal, currentMatches, state2.loaderData), resolveFetcherDeferredResults2(matches, fetcherResults, fetchersToLoad)]);
    return {
      loaderResults,
      fetcherResults
    };
  ...
  function checkFogOfWar(matches, routesToUse, pathname) {
    if (patchRoutesOnNavigationImpl) {
      if (!matches) {
        let fogMatches = matchRoutesImpl2(routesToUse, pathname, basename, true);
        return {
          active: true,
          matches: fogMatches || []
        };
        ...
        if (Object.keys(matches[0].params).length > 0) {
          let partialMatches = matchRoutesImpl2(routesToUse, pathname, basename, true);
          return {
            active: true,
            matches: partialMatches
          };
    ...
    while (true) {
      ...
      try {
        await patchRoutesOnNavigationImpl({
          ...
          patch: (routeId, children) => {
            if (signal.aborted) return;
            patchRoutesImpl2(routeId, children, routesToUse, localManifest, mapRouteProperties2);
          }
        });
      } catch (e) {
        return {
      ...
      let newMatches = matchRoutes2(routesToUse, pathname, basename);
      if (newMatches) {
        return {
          type: "success",
          matches: newMatches
      ...
      let newPartialMatches = matchRoutesImpl2(routesToUse, pathname, basename, true);
      if (!newPartialMatches || partialMatches.length === newPartialMatches.length && partialMatches.every((m, i) => m.route.id === newPartialMatches[i].route.id)) {
        return {
          type: "success",
          matches: null
  ...
  function patchRoutes(routeId, children) {
    let isNonHMR = inFlightDataRoutes == null;
    let routesToUse = inFlightDataRoutes || dataRoutes;
    patchRoutesImpl2(routeId, children, routesToUse, manifest, mapRouteProperties2);
    if (isNonHMR) {
      dataRoutes = [...dataRoutes];
      updateState({});
    }
...
function normalizeTo2(location, matches, basename, prependBasename, to, v7_relativeSplatPath, fromRouteId, relative) {
  ...
  if ((to == null || to === "" || to === ".") && activeRouteMatch) {
    let nakedIndex = hasNakedIndexQuery2(path.search);
    if (activeRouteMatch.route.index && !nakedIndex) {
      path.search = path.search ? path.search.replace(/^\?/, "?index&") : "?index";
    } else if (!activeRouteMatch.route.index && nakedIndex) {
      let params = new URLSearchParams(path.search);
...
function getLoaderMatchesUntilBoundary2(matches, boundaryId, includeBoundary) {
  if (includeBoundary === void 0) {
    includeBoundary = false;
  }
  let index = matches.findIndex((m) => m.route.id === boundaryId);
...
function getMatchesToLoad2(history, state, matches, submission, location, initialHydration, skipActionErrorRevalidation, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionResult) {
  let actionResult = pendingActionResult ? isErrorResult2(pendingActionResult[1]) ? pendingActionResult[1].error : pendingActionResult[1].data : void 0;
  let currentUrl = history.createURL(state.location);
  let nextUrl = history.createURL(location);
  let boundaryMatches = matches;
  if (initialHydration && state.errors) {
    boundaryMatches = getLoaderMatchesUntilBoundary2(matches, Object.keys(state.errors)[0], true);
  } else if (pendingActionResult && isErrorResult2(pendingActionResult[1])) {
    boundaryMatches = getLoaderMatchesUntilBoundary2(matches, pendingActionResult[0]);
  }
  let actionStatus = pendingActionResult ? pendingActionResult[1].statusCode : void 0;
  let shouldSkipRevalidation = skipActionErrorRevalidation && actionStatus && actionStatus >= 400;
  let navigationMatches = boundaryMatches.filter((match, index) => {
    ...
    if (initialHydration) {
      return shouldLoadRouteOnHydration2(route, state.loaderData, state.errors);
    }
...
function shouldLoadRouteOnHydration2(route, loaderData, errors) {
  if (route.lazy) {
    return true;
  }
  if (!route.loader) {
...
function patchRoutesImpl2(routeId, children, routesToUse, manifest, mapRouteProperties2) {
  var _childrenToPatch;
  let childrenToPatch;
  if (routeId) {
    let route = manifest[routeId];
  ...
  let uniqueChildren = children.filter((newRoute) => !childrenToPatch.some((existingRoute) => isSameRoute2(newRoute, existingRoute)));
  let newRoutes = convertRoutesToDataRoutes2(uniqueChildren, mapRouteProperties2, [routeId || "_", "patch", String(((_childrenToPatch = childrenToPatch) == null ? void 0 : _childrenToPatch.length) || "0")], manifest);
  childrenToPatch.push(...newRoutes);
}
function isSameRoute2(newRoute, existingRoute) {
  if ("id" in newRoute && "id" in existingRoute && newRoute.id === existingRoute.id) {
    return true;
  }
  if (!(newRoute.index === existingRoute.index && newRoute.path === existingRoute.path && newRoute.caseSensitive === existingRoute.caseSensitive)) {
  ...
  return newRoute.children.every((aChild, i) => {
    var _existingRoute$childr;
    return (_existingRoute$childr = existingRoute.children) == null ? void 0 : _existingRoute$childr.some((bChild) => isSameRoute2(aChild, bChild));
  });
}
...
async function defaultDataStrategy2(_ref4) {
  let {
    matches
  } = _ref4;
  let matchesToLoad = matches.filter((m) => m.shouldLoad);
...
async function callDataStrategyImpl2(dataStrategyImpl, type, state, request, matchesToLoad, matches, fetcherKey, manifest, mapRouteProperties2, requestContext) {
  let loadRouteDefinitionsPromises = matches.map((m) => m.route.lazy ? loadLazyRouteModule2(m.route, mapRouteProperties2, manifest) : void 0);
  let dsMatches = matches.map((match, i) => {
    let loadRoutePromise = loadRouteDefinitionsPromises[i];
    let shouldLoad = matchesToLoad.some((m) => m.route.id === match.route.id);
    let resolve = async (handlerOverride) => {
      ...
      return shouldLoad ? callLoaderOrAction2(type, request, match, loadRoutePromise, handlerOverride, requestContext) : Promise.resolve({
        type: ResultType2.data,
        result: void 0
      });
    };
    return _extends3({}, match, {
      shouldLoad,
      resolve
    });
  });
...
async function callLoaderOrAction2(type, request, match, loadRoutePromise, handlerOverride, staticContext) {
  let result;
  let onReject;
  let runHandler = (handler) => {
    let reject;
  ...
  try {
    ...
    invariant2(result.result !== void 0, "You defined " + (type === "action" ? "an action" : "a loader") + " for route " + ('"' + match.route.id + "\" but didn't return anything from your `" + type + "` ") + "function. Please return a value or `null`.");
  } catch (e) {
    return {
      type: ResultType2.error,
      result: e
...
async function convertDataStrategyResultToDataResult2(dataStrategyResult) {
  let {
    result,
    type
  } = dataStrategyResult;
  if (isResponse2(result)) {
    ...
    if (type === ResultType2.error) {
      return {
        type: ResultType2.error,
        error: new ErrorResponseImpl2(result.status, result.statusText, data),
        statusCode: result.status,
        headers: result.headers
      };
    }
  ...
  if (type === ResultType2.error) {
    if (isDataWithResponseInit2(result)) {
      var _result$init3, _result$init4;
      if (result.data instanceof Error) {
        var _result$init, _result$init2;
        return {
      ...
      return {
        type: ResultType2.error,
        error: new ErrorResponseImpl2(((_result$init3 = result.init) == null ? void 0 : _result$init3.status) || 500, void 0, result.data),
...
function normalizeRelativeRoutingRedirectResponse2(response, request, routeId, matches, basename, v7_relativeSplatPath) {
  let location = response.headers.get("Location");
  ...
  if (!ABSOLUTE_URL_REGEX2.test(location)) {
    let trimmedMatches = matches.slice(0, matches.findIndex((m) => m.route.id === routeId) + 1);
    location = normalizeTo2(new URL(request.url), trimmedMatches, basename, true, location, v7_relativeSplatPath);
    response.headers.set("Location", location);
  }
  return response;
}
function normalizeRedirectLocation2(location, currentUrl, basename, historyInstance) {
  let invalidProtocols = [
    "about:",
    "blob:",
    "chrome:",
  ...
  if (ABSOLUTE_URL_REGEX2.test(location)) {
    ...
    if (url.origin === currentUrl.origin && isSameBasename) {
      return removeDoubleSlashes2(url.pathname) + url.search + url.hash;
    }
  }
  try {
    let url = historyInstance.createURL(location);
...
function processRouteLoaderData2(matches, results, pendingActionResult, activeDeferreds, skipLoaderErrorBubbling) {
  let loaderData = {};
  let errors = null;
  let statusCode;
  let foundError = false;
  let loaderHeaders = {};
  let pendingError = pendingActionResult && isErrorResult2(pendingActionResult[1]) ? pendingActionResult[1].error : void 0;
  matches.forEach((match) => {
    if (!(match.route.id in results)) {
      return;
    }
...
function processLoaderData2(state, matches, results, pendingActionResult, revalidatingFetchers, fetcherResults, activeDeferreds) {
  let {
    loaderData,
    errors
  } = processRouteLoaderData2(
    matches,
    results,
    pendingActionResult,
    activeDeferreds,
...
function getActionDataForCommit2(pendingActionResult) {
  if (!pendingActionResult) {
    return {};
  }
  return isErrorResult2(pendingActionResult[1]) ? {
    // Clear out prior actionData on errors
    actionData: {}
  } : {
    actionData: {
...
function isRedirectDataStrategyResultResult2(result) {
  return isResponse2(result.result) && redirectStatusCodes2.has(result.result.status);
}
...
function isDataWithResponseInit2(value) {
  return typeof value === "object" && value != null && "type" in value && "data" in value && "init" in value && value.type === "DataWithResponseInit";
}
...
async function resolveNavigationDeferredResults2(matches, results, signal, currentMatches, currentLoaderData) {
  let entries = Object.entries(results);
  for (let index = 0; index < entries.length; index++) {
    let [routeId, result] = entries[index];
    let match = matches.find((m) => (m == null ? void 0 : m.route.id) === routeId);
    ...
    if (isDeferredResult2(result) && isRevalidatingLoader) {
      await resolveDeferredData2(result, signal, false).then((result2) => {
        if (result2) {
          results[routeId] = result2;
        }
      });
...
async function resolveFetcherDeferredResults2(matches, results, revalidatingFetchers) {
  for (let index = 0; index < revalidatingFetchers.length; index++) {
    let {
      key,
      routeId,
    ...
    if (isDeferredResult2(result)) {
      ...
      await resolveDeferredData2(result, controller.signal, true).then((result2) => {
        if (result2) {
          results[key] = result2;
        }
      });
...
function persistAppliedTransitions2(_window, transitions) {
  if (transitions.size > 0) {
    ...
    try {
      _window.sessionStorage.setItem(TRANSITIONS_STORAGE_KEY2, JSON.stringify(json3));
    } catch (error) {
...
function useSearchParams(defaultInit) {
  true ? warning2(typeof URLSearchParams !== "undefined", "You cannot use the `useSearchParams` hook in a browser that does not support the URLSearchParams API. If you need to support Internet Explorer 11, we recommend you load a polyfill such as https://github.com/ungap/url-search-params.") : void 0;
...
function useFetcher(_temp3) {
  ...
  let FetcherForm = React2.useMemo(() => {
    let FetcherForm2 = React2.forwardRef((props, ref) => {
      return React2.createElement(Form, _extends4({}, props, {
        navigate: false,
        fetcherKey,
        ref
      }));
...
function useViewTransitionState(to, opts) {
  ...
  !(vtContext != null) ? true ? invariant2(false, "`useViewTransitionState` must be used within `react-router-dom`'s `RouterProvider`.  Did you accidentally import `RouterProvider` from `react-router`?") : invariant2(false) : void 0;
  let {
    basename
...
export {
  ...
  ErrorResponseImpl2 as UNSAFE_ErrorResponseImpl,
